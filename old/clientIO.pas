...
type
	...
	PConnInfo = ^TConnInfo;
	TConnInfo = record
		// Deze dingen moeten ingesteld worden
		Username:	string;
		Gebied:		string;
		simnaam:		string;
		simver:		string;
		// Dit zijn interne gegevens
		f1lastsent:	string;
		fase:			integer;
	end;

	pClient = ^tClient;
	TReadMsgEvent = procedure(ConnInfo: PConnInfo; msg: string) of object;
	...
	public
		// Dit event MOET ingesteld worden:
		ReadMsg		:  TReadMsgEvent;
		// Voor het connecten moet ConnInfo van de juiste gegevens worden
		// voorzien:
		ConnInfo		:  TConnInfo;
...
begin
	...
	OnSessionConnected := SessionConnected;
...
procedure TClient.SessionConnected;
begin
	// Stuur eens de gebruikersnaam om alles aan het rollen te brengen.
	SendString('user:'+ConnInfo.Username);
	ConnInfo.f1lastsent := 'user';
end;
